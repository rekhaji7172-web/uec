...
export default function AdminSignUpPage() {
  ...
  async function submit(form: FormData) { setPending(true); setError(""); const response = await fetch("/api/admin/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.get("name"), password: form.get("password") }) }); if (!response.ok) { setError("Use the dashboard Add admin control after signing in."); setPending(false); return }; router.push("/admin"); router.refresh() }
