    ...
    <section className="developer-credit-section" aria-labelledby="developer-credit-title">
      <div className="developer-credit-card">
        ...
        <div className="developer-credit-copy">
          ...
          <p className="developer-bio">Hello I am Yuvraj! I designed and built this UEC website to give every editor, season and story a place to shine.</p>
      ...
      {contactOpen && <div className="developer-contact-backdrop" role="presentation" onClick={() => setContactOpen(false)}><div className="developer-contact-dialog" role="dialog" aria-modal="true" aria-labelledby="contact-dialog-title" onClick={(event) => event.stopPropagation()}><button type="button" className="dialog-close" aria-label="Close contact dialog" onClick={() => setContactOpen(false)}>×</button><span className="eyebrow">Let&apos;s connect</span><h3 id="contact-dialog-title">Talk to Yuvraj</h3><p>My Discord username is <strong>YchessEditz</strong>. Send a friend request to talk.</p><div className="discord-handle"><span>DISCORD</span><strong>YchessEditz</strong></div><button type="button" className="developer-contact-button" onClick={() => navigator.clipboard?.writeText("YchessEditz")}>Copy username</button></div></div>}
    </section>
  )
}
